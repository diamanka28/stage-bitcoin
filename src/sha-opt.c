...
do {\
    ...
    w[i + 2] = SIGMA3(w[i]) + w[i - 5] + SIGMA2(w[i - 13]) + w[i - 14];\
    w[i + 3] = SIGMA3(w[i + 1]) + w[i - 4] + SIGMA2(w[i - 12]) + w[i - 13];\
    w[i + 4] = SIGMA3(w[i + 2]) + w[i - 3] + SIGMA2(w[i - 11]) + w[i - 12];\
    w[i + 5] = SIGMA3(w[i + 3]) + w[i - 2] + SIGMA2(w[i - 10]) + w[i - 11];\
    w[i + 6] = SIGMA3(w[i + 4]) + w[i - 1] + SIGMA2(w[i - 9]) + w[i - 10];\
    w[i + 7] = SIGMA3(w[i + 5]) + w[i] + SIGMA2(w[i - 8]) + w[i - 9];\
} while(0)
...
uint8_t *sha256_opt(uint8_t *input, size_t len){
    ...
    size_t num_blocks = (len + 1 + padding_len + 8) >> 6;
    ...
    SPLIT_8BIT(digest, H[0], 0);
    SPLIT_8BIT(digest, H[1], 4);
    SPLIT_8BIT(digest, H[2], 8);
    SPLIT_8BIT(digest, H[3], 12);
    SPLIT_8BIT(digest, H[4], 16);
    SPLIT_8BIT(digest, H[5], 20);
    SPLIT_8BIT(digest, H[6], 24);
    SPLIT_8BIT(digest, H[7], 28);
