...
#ifdef DPU
extern uint8_t *sha256_opt(uint8_t *input, size_t len);
...
void hash_rate_test(uint32_t nb_iterations){
    uint8_t hash[CONCAT_LENGTH]={"Mamadou Diamanka"};
    uint8_t input[256] = "ffffffffffffffffffffffffffffffffffffffffffffffffff";
    //size_t len = strlen(input);
    for(uint32_t i = 0; i < nb_iterations; i++){

#if OPTIMISED
        // sha256(hash,CONCAT_LENGTH,hash);
        // sha256(hash,SIZE_OF_SHA_256_HASH,hash);   
        sha256_opt(sha256_opt(input,60),64);
#else
        calc_sha_256(hash,hash,CONCAT_LENGTH);
        calc_sha_256(hash,hash,SIZE_OF_SHA_256_HASH);
#endif    
